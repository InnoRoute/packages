...
# Accepts none or one parameter. Parameter given: 1=short, 2=extra verbose, otherwise: normal verbosity
...
let    tm_buffer_full=`TNbar1 $(($C_BASE_ADDR_COMMON_LOWER*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_COMMON_BUFFER_FULL)) | cut -d " " -f 6`
...
if [[ $1 -eq 2 ]]; then
  ...
  for clk in `seq 0 $last_clk`; do
    let clk_freq=`TNbar1 $(($C_BASE_ADDR_COMMON_LOWER*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_COMMON_CLK_MON_L+4*$clk)) | cut -d " " -f 6`
    case $clk in
      0)
  ...
  let    peripherals_interrupt_bitmap=`TNbar1 $(($C_BASE_ADDR_MDIO*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_MDIO_INTERRUPT)) | cut -d " " -f 6`
  ...
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_0*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 0: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_1*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 1: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_2*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 2: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_3*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 3: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_4*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 4: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_5*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 5: %d\n" $phy_prt
  ...
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_6*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 6: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_7*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 7: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_8*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 8: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_9*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 9: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_10*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 10: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_11*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 11: %d\n" $phy_prt
  ...
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_PCIE_0*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* PCIe 0: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_PCIE_1*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_INTERRUPT)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* PCIe 1: %d\n" $phy_prt
  ...
  echo -e "\n### Displaying non-zero fragment dropping counts at HC modules"

  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_0*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 0: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_1*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 1: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_2*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 2: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_3*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 3: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_4*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 4: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_5*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 5: %d\n" $phy_prt
  ...
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_6*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 6: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_7*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 7: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_8*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 8: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_9*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 9: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_10*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 10: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_11*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* Port 11: %d\n" $phy_prt
  ...
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_PCIE_0*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* PCIe 0: %d\n" $phy_prt
  fi
  let phy_prt=`TNbar1 $(($C_BASE_ADDR_HC_PCIE_1*$C_BASE_ADDR_FACTOR+$C_SUB_ADDR_HC_CNT_BAD_FRAMES)) | cut -d " " -f 6`
  if [[ $phy_prt -eq 0xEEEEEEEE ]]; then
    echo " ** MMI Read Timeout **"
  elif [[ $phy_prt -gt 0 ]]; then
    printf "* PCIe 1: %d\n" $phy_prt
  ...
  echo -e "\n### Displaying non-zero frame dropping counts at RX Statistics units"
  echo "# Datapath 0"
  for prt in `seq 0 31`; do
    for badrsn in `seq 0 31`; do
      ...
      if [[ $badrsn_cnt -eq 0xEEEEEEEE ]]; then
        echo " ** MMI Read Timeout **"
      elif [[ $badrsn_cnt -gt 0 ]]; then
        printf "* Port %d - BadReason %d: %d\n" $prt $badrsn $badrsn_cnt
      fi
    done
  done
  echo "# Datapath 1"
