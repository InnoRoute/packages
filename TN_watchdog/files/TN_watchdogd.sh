...
do
	cat /proc/TrustNode/trigger_MMI_int > /dev/null 
	if [ "$(cat /proc/TrustNode/TSN/PTP_clockjump )" -gt "0" ]; then
		TNtsnctl refresh  #refresh TAS if there was a clockjump
		echo "$datum TAS refreshed"
	fi
	#polling tn_mmi interrupt
