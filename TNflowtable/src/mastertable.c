...
//************************************************************************************************************************************
/**
*return return howoften an actiontable entry is used
*@param arguments struct with request
*/
uint64_t
...
verblog  printf ("__FUNCTION__ = %s\n", __FUNCTION__);
  ...
  do {
    ...
    entry = (struct arguments *) INR_MasterT_get_addr (i++);
    if (entry->TableID.ActT == actid && entry->used == 1)count++;
...
{
verblog  printf ("__FUNCTION__ = %s\n", __FUNCTION__);
if (arguments->ID)return arguments->ID;//if if provided, we neednt to search :P
  uint64_t i = 0;
uint8_t match=1;
  struct arguments *entry = NULL;
  do {
    entry = (struct arguments *) INR_MasterT_get_addr (i++);
	match=entry->used;//if unused.. don't match
...
verblog	if(match)printf("Entry fund, ID:%li\n");
       if (match)return i-1;
  } while ((i < MASTERTABLE_length));
...
verblog  printf ("__FUNCTION__ = %s\n", __FUNCTION__);
  ...
  if (entry_is_EMH2 (arguments))
    arguments->TYPE_ID = 0;//2  type2 actually deactivated..	// set type
if(arguments->dohave_INPORT)arguments->TYPE_ID = 0;//if inport is present it is EMA
...
if(INR_MasterT_get_next_free_entry (arguments->ID)){
  ...
  memcpy (entry, arguments, sizeof (struct arguments));	//thats easy :D
}else printf("MasterTabele full");
  return 0;
}
...
//remove entry from tables where stored
  if (entry->TableID.EMH_RT)INR_RuleTable_EMH_clear_entry(entry->TableID.EMH_RT);
  if (entry->TableID.EMH_HT)INR_HashTable_EMH_clear_entry(entry->TableID.EMH_HT);
  if (entry->TableID.EMH_CT)INR_CTable_EMH_clear_entry(entry->TableID.EMH_CT);
  if (entry->TableID.EMA_RT)INR_RuleTable_EMA_clear_entry(entry->TableID.EMA_RT);
  if (entry->TableID.EMA_HT)INR_HashTable_EMA_clear_entry(entry->TableID.EMA_HT);
  if (entry->TableID.ActT)if(FC_MT_count_action(entry->TableID.ActT)==1)INR_ActT_clear_entry(entry->TableID.ActT);
//can't remove because multible entrys could have the same action... first check'
  entry->used = 0;
}
  return ID;
}
...
//  printf ("MT addr request for id:%i addr:0x%lx\n", id, addr);
  return MAbase + (addr);
}
...
FC_MasterT_print (struct arguments *arguments)
{
  ...
  for (i = arguments->ID; ((i < MASTERTABLE_length) && (i < arguments->ID + arguments->COUNT)); i++) {
	...
	if (entry->TableID.ActT)printf("ActT:%li ",entry->TableID.ActT);}
	printf ("\n");
    }
    else
      printf ("ID not valid\n");
