/**
*@file tnlibqcictl.c
*@brief userspace QCI_QCI acces tool
*M.Ulbricht 2019
**/
...
uint8_t TN_qci_verbose = 0;
uint64_t BASE = 0;
uint64_t BASE_shadow = 0;
uint64_t BASE_shadow2 = 0;
uint8_t TN_qci_MEMDUMP = 0;
uint8_t QCI_enable=1;
uint8_t QCI_PORT_count=0;
uint8_t PORT_count=0;

uint32_t	TASvar_QCI_config_space_lower=0;
uint32_t	TASvar_PORT_QCI_width=0;
...
uint32_t  TASvar_IPV_base=0;
uint32_t  TASvar_IPV_entry_width=0;
...
*/
void
TN_qci_clear_arguments (struct arguments *arguments)
{
  arguments->args[0] = "";
  arguments->verbose = 0;
  arguments->used = 0;
  ...
  arguments->IPV_value= 0;
  arguments->IPV_gate= 0;
  arguments->IPV_gclenty= 0;

  arguments->dohave_IPV_value= 0;
  arguments->dohave_IPV_gate= 0;
  arguments->dohave_IPV_gclenty= 0;  
...
if (QCI_enable) {
	...
	if (TN_qci_verbose)
    printf ("info: %s:%d:%s \n", __FILE__, __LINE__, __func__,get_QCI_PORT_count());
	uint8_t port=0;
	uint16_t gcl_entry=0;
	for(port=0;port<get_QCI_PORT_count();port++){
		if(QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_GATE_ENABLE,port,0)){
		if (TN_qci_verbose)printf("refresh GCL %i %i\n",port,gcl_entry);
		QCI_set_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN,port,QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN,port,0));
		for(gcl_entry=0;gcl_entry<QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN,port,0);gcl_entry++){
					if (TN_qci_verbose)printf("refresh GCL %i %i\n",port,gcl_entry);
			    struct GateControl_list_entry *entry =
     		 		BASE + port * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * gcl_entry;
			    struct GateControl_list_entry *entry_shadow =
 				     BASE_shadow + port * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * gcl_entry;
			    TSNmemcpy (entry, entry_shadow, TASvar_GateControl_list_entry_length);
			    
					struct GateControl_TIME_list_entry *Tentry =
     			 BASE + port * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * gcl_entry;
    			struct GateControl_TIME_list_entry *Tentry_shadow =
     			 BASE_shadow + port * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * gcl_entry;
     			 
     			struct granularitytime *correcttime =
      		BASE_shadow2 + port * TASvar_PORT_QCI_width * 2 + sizeof(struct granularitytime) + sizeof(struct granularitytime) * gcl_entry;
   				 if ((entry_shadow != NULL)&&(correcttime != NULL)) {
    				*correcttime=QCI_time_ganularity(Tentry_shadow->time);
    			}
		...
		arguments.ADMIN_BASE_TIME=QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_BASE_TIME,port,1);
		arguments.dohave_ADMIN_BASE_TIME=1;
		arguments.ADMIN_CYCLE_TIME=QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_CYCLE_TIME,port,0);
		arguments.dohave_ADMIN_CYCLE_TIME=1;
		arguments.ADMIN_CYCLE_TIME_EXT=QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_CYCLE_TIME_EXT,port,0);
		arguments.dohave_ADMIN_CYCLE_TIME_EXT=1;
		arguments.GATE_ENABLE=QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_GATE_ENABLE,port,0);
		arguments.dohave_GATE_ENABLE=1;
		arguments.dohave_PORT=1;
		new_QCI_apply (&arguments);


	}else {
	if (TN_qci_verbose)printf("port %i disabled\n",port);
	}
...
TN_qci_printallconst ()
{
  ...
  printconst (TASvar_QCI_config_space_lower);
  printconst (get_QCI_PORT_count());
  printconst (TASvar_PORT_QCI_width);
...
INR_TAS=1; // ok, we have no TAS her but i am to lazy to change the names
	...
	QCI_PORT_count=1;
#endif
 	TASvar_QCI_config_space_lower=(C_BASE_ADDR_QCI0_LOWER<<8);
	TASvar_PORT_QCI_width=0; //actually there is just one block, so wee need maybe later
	TASvar_GateControl_list_base=(C_SUB_ADDR_QCI0_SGCL_LOWER<<0)+(TASvar_QCI_config_space_lower);
	TASvar_GateControl_list_entry_length=4;
	TASvar_GateControl_list_length=((((C_SUB_ADDR_QCI0_SGCL_UPPER+1)<<0)-(C_SUB_ADDR_QCI0_SGCL_LOWER<<0))/TASvar_GateControl_list_entry_length);
	TASvar_GateControl_TIME_list_base=(C_SUB_ADDR_QCI0_SGCL_TIME_LOWER<<0)+(TASvar_QCI_config_space_lower);
	TASvar_GateControl_TIME_list_entry_length=4;
	TASvar_GateControl_TIME_list_length=((((C_SUB_ADDR_QCI0_SGCL_TIME_UPPER+1)<<0)-(C_SUB_ADDR_QCI0_SGCL_TIME_LOWER<<0))/TASvar_GateControl_TIME_list_entry_length);

	TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN=C_SUB_ADDR_QCI0_ADMIN_SGCL_LENGTH;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_BASE_TIME=C_SUB_ADDR_QCI0_ADMIN_BASE_TIME;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_CYCLE_TIME=C_SUB_ADDR_QCI0_ADMIN_CYCLE_TIME;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_CYCLE_TIME_EXT=C_SUB_ADDR_QCI0_ADMIN_CYCLE_TIME_EXTENSION;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_CONFIG_CHANGE_TIME=C_SUB_ADDR_QCI0_CONFIG_CHANGE_TIME;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_CYCLE_START_TIME=0;// not used for this tas
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_GATE_ENABLE=C_SUB_ADDR_QCI0_GATE_ENABLE;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_CONFIG_CHANGE=C_SUB_ADDR_QCI0_CONFIG_CHANGE;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_CONFIG_CHANGE_PENDING=C_SUB_ADDR_QCI0_CONFIG_PENDING;

  TASvar_C_SUB_ADDR_TM_SCHED_TAS_OPER_GCL_LEN=0; // not used
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_OPER_BASE_TIME=C_SUB_ADDR_QCI0_OPER_BASE_TIME;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_OPER_CYCLE_TIME=C_SUB_ADDR_QCI0_OPER_CYCLE_TIME;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_OPER_CYCLE_TIME_EXT=C_SUB_ADDR_QCI0_OPER_CYCLE_TIME_EXTENSION;
  TASvar_C_SUB_ADDR_TM_SCHED_TAS_OPER_GATE_STATES=C_SUB_ADDR_QCI0_OPER_GATE_STATES;
   TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GATE_STATES=C_SUB_ADDR_QCI0_ADMIN_STREAM_GATE_STATES;
   
   TASvar_IPV_base=(C_BASE_ADDR_QCI1_LOWER<<8)+C_SUB_ADDR_QCI1_IPV_LOWER;
   TASvar_IPV_entry_width=4; //C_HEADER_FLOWID_WIDTH_TM
...
*/
void
QCI_init (uint64_t * base, uint64_t * shadow_base,uint64_t * shadow_base2)
{
  BASE = (uint64_t) base;
...
*/
void
TN_qci_config_change (struct arguments *arguments)
{
  if (TN_qci_verbose)
    printf ("info: %s:%d:%s \n", __FILE__, __LINE__, __func__);
  verblog printf ("Wait until pending config is done\n");
    while (QCI_get_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_CONFIG_CHANGE_PENDING, arguments->PORT, 0)) {
      POLLSLEEP}
  if (QCI_enable) {
    QCI_config_dohavewrite (arguments, ADMIN_GCL_LEN)
      QCI_config_dohavewrite_time (arguments, ADMIN_BASE_TIME)
      QCI_config_dohavewrite_time (arguments, ADMIN_CYCLE_TIME)
      QCI_config_dohavewrite_time (arguments, ADMIN_CYCLE_TIME_EXT)
      QCI_config_dohavewrite_time (arguments, CONFIG_CHANGE_TIME)
      QCI_config_dohavewrite_time (arguments, CYCLE_START_TIME)
      QCI_config_dohavewrite (arguments, GATE_ENABLE)
      QCI_config_dohavewrite (arguments, CONFIG_CHANGE)
      QCI_config_dohavewrite (arguments, ADMIN_GATE_STATES)
//readonly!: QCI_config_dohavewrite(arguments,CONFIG_CHANGE_PENDING)
//readonly!: QCI_config_dohavewrite(arguments,CONFIG_CHANGE_ACK)
//readonly!: QCI_config_dohavewrite(arguments,OPER_GCL_LEN)
//readonly!: QCI_config_dohavewrite(arguments,OPER_BASE_TIME)
//readonly!: QCI_config_dohavewrite(arguments,OPER_CYCLE_TIME)
//readonly!: QCI_config_dohavewrite(arguments,OPER_CYCLE_TIME_EXT)
  }
};
...
config_print (struct arguments *arguments)
{
  if (TN_qci_verbose)
    printf ("info: %s:%d:%s \n", __FILE__, __LINE__, __func__);
  if (QCI_enable) {
    QCI_config_print (ADMIN_GCL_LEN, 0)
      QCI_config_print (ADMIN_BASE_TIME, 0)
      QCI_config_print (ADMIN_CYCLE_TIME, 0)
      QCI_config_print (ADMIN_CYCLE_TIME_EXT, 0)
      QCI_config_print (CONFIG_CHANGE_TIME, 0)
      QCI_config_print (CYCLE_START_TIME, 0)
      QCI_config_print (GATE_ENABLE, 0)
      QCI_config_print (CONFIG_CHANGE, 0)
      QCI_config_print (CONFIG_CHANGE_PENDING, 0)
      QCI_config_print (OPER_GCL_LEN, 0)
      QCI_config_print (OPER_BASE_TIME, 0)
      QCI_config_print (OPER_CYCLE_TIME, 0)
      QCI_config_print (OPER_CYCLE_TIME_EXT, 0)
      QCI_config_print (OPER_GATE_STATES, 0)
      //QCI_config_print (IPV_entry_width,1)

      //QCI_config_print (ADMIN_GATE_STATES, 0)
  }
};
//********************************************************************************************************************
/**
...
GCL_entry (struct arguments *arguments)
{
  ...
  if(arguments->PORT>=QCI_PORT_count){
  ...
  if (QCI_enable) {
    ...
    TN_qci_config_change (arguments);//write config
    
    
    if (QCI_get_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN, arguments->PORT, 0)<=arguments->ID) {
      printf ("Provided ID(%li) is beyond the GateControlList length (%li)\n",arguments->ID,QCI_get_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN, arguments->PORT, 0));
      return;
    }
...
*/
void
TN_qci_GateControl_list_change (struct arguments *arguments)
{
  if (TN_qci_verbose)
    printf ("info: %s:%d:%s \n", __FILE__, __LINE__, __func__);
  if (QCI_enable) {
    ...
    if (TN_qci_verbose)
      printf ("accessing address:0x%llx\n", arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * arguments->ID);
    struct GateControl_list_entry *entry =
      BASE + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * arguments->ID;
    /*if (entry != NULL) {
       entry->GCL=arguments->GATE_STATE_VECTOR;
       } */
    struct GateControl_list_entry *entry_shadow =
      BASE_shadow + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * arguments->ID;
...
*/
void
TN_qci_IPV_list_change (struct arguments *arguments)
{
  if (TN_qci_verbose)
    printf ("info: %s:%d:%s \n", __FILE__, __LINE__, __func__);
  if (QCI_enable) {
  ...
  if(arguments->dohave_IPV_value==0){
  	printf ("please provide a IPV_value!\n");
    return;
  }
  if(arguments->dohave_IPV_gate==0){
  	printf ("please provide a IPV_gate!\n");
    return;
  }
  if(arguments->dohave_IPV_gclenty==0){
  	printf ("please provide a IPV_gclenty!\n");
    return;
  }
    ...
    if (TN_qci_verbose)
      printf ("accessing address:0x%llx\n", arguments->PORT * TASvar_PORT_QCI_width + TASvar_IPV_base +(((uint32_t)arguments->IPV_gate&0x1F)<<2)+(((uint32_t)arguments->IPV_gclenty&0x7F)<<6));
    uint32_t *entry =
      BASE + arguments->PORT * TASvar_PORT_QCI_width + TASvar_IPV_base +(((uint32_t)arguments->IPV_gate&0x1F)<<2)+(((uint32_t)arguments->IPV_gclenty&0x7F)<<6);

    uint32_t *entry_shadow =
      BASE_shadow + arguments->PORT * TASvar_PORT_QCI_width + TASvar_IPV_base +(((uint32_t)arguments->IPV_gate&0x1F)<<2)+(((uint32_t)arguments->IPV_gclenty&0x7F)<<6);
    if (entry_shadow != NULL) {
      *entry_shadow = arguments->IPV_value;
    }
    TSNmemcpy (entry, entry_shadow, TASvar_IPV_entry_width);
...
GateControl_list_print (struct arguments *arguments)
{
  ...
  if (QCI_enable) {
    ...
    for (i = arguments->ID; ((i < TASvar_GateControl_list_length) && (i < arguments->ID + arguments->COUNT)); i++) {
      if (TN_qci_verbose)
	printf ("accessing address[%i]:0x%llx\n", i, arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * i);
      struct GateControl_list_entry *entry =
	BASE_shadow + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * i;
      if (entry != NULL) {
	MACHINEREADABLE {
...
struct granularitytime get_QCI_granularitytime(uint8_t port, uint32_t ID){
	...
	struct granularitytime *correcttime = BASE_shadow2 + port * TASvar_PORT_QCI_width * 2 + sizeof(struct granularitytime) + sizeof(struct granularitytime) * ID;
	retval=*correcttime;
...
*/
void
TN_qci_GateControl_TIME_list_change (struct arguments *arguments)
{
  ...
  if (QCI_enable) {
    ...
    if (TN_qci_verbose)
      printf ("accessing address:0x%llx\n",
	      arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * arguments->ID);
    struct GateControl_TIME_list_entry *entry =
      BASE + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * arguments->ID;
    /*if (entry != NULL) {
       entry->time=arguments->INTERVAL;
       } */
    struct GateControl_TIME_list_entry *entry_shadow =
      BASE_shadow + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * arguments->ID;
    struct granularitytime *correcttime =
      BASE_shadow2 + arguments->PORT * TASvar_PORT_QCI_width * 2 + sizeof(struct granularitytime) + sizeof(struct granularitytime) * arguments->ID;
    if ((entry_shadow != NULL)&&(correcttime != NULL)) {
    	*correcttime=QCI_time_ganularity(arguments->INTERVAL);
...
GateControl_TIME_list_print (struct arguments *arguments)
{
  ...
  if (QCI_enable) {
    ...
    for (i = arguments->ID; ((i < TASvar_GateControl_TIME_list_length) && (i < arguments->ID + arguments->COUNT)); i++) {
      if (TN_qci_verbose)
	printf ("accessing address[%i]:0x%llx\n", i,
		arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * i);
      struct GateControl_TIME_list_entry *entry =
	BASE_shadow + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * i;
      if (entry != NULL) {
...
QUEUE_PRIO_list_change (struct arguments *arguments)
{
  ...
  if (QCI_enable) {
    ...
    if (TN_qci_verbose)
      printf ("accessing address:0x%llx\n", arguments->PORT * TASvar_PORT_QCI_width + TASvar_QUEUE_PRIO_list_base + TASvar_QUEUE_PRIO_list_entry_length * arguments->ID);
    struct QUEUE_PRIO_list_entry *entry =
      BASE + arguments->PORT * TASvar_PORT_QCI_width + TASvar_QUEUE_PRIO_list_base + TASvar_QUEUE_PRIO_list_entry_length * arguments->ID;
    /*if (entry != NULL) {
       entry->prio=arguments->QUEUE_PRIO;
       } */
    struct QUEUE_PRIO_list_entry *entry_shadow =
      BASE_shadow + arguments->PORT * TASvar_PORT_QCI_width + TASvar_QUEUE_PRIO_list_base + TASvar_QUEUE_PRIO_list_entry_length * arguments->ID;
    if (entry_shadow != NULL) {
...
QUEUE_PRIO_list_print (struct arguments *arguments)
{
  ...
  if (QCI_enable) {
    ...
    for (i = arguments->ID; ((i < TASvar_QUEUE_PRIO_list_length) && (i < arguments->ID + arguments->COUNT)); i++) {
      if (TN_qci_verbose)
	printf ("accessing address[%i]:0x%llx\n", i, arguments->PORT * TASvar_PORT_QCI_width + TASvar_QUEUE_PRIO_list_base + TASvar_QUEUE_PRIO_list_entry_length * i);
      struct QUEUE_PRIO_list_entry *entry = BASE_shadow + arguments->PORT * TASvar_PORT_QCI_width + TASvar_QUEUE_PRIO_list_base + TASvar_QUEUE_PRIO_list_entry_length * i;
      if (entry != NULL) {
...
*/
uint32_t
QCI_get_config (uint64_t reg, uint8_t port, uint8_t shadow)
{
  if (QCI_enable) {
    ...
    if (shadow)
      ...
      base = BASE;
    uint32_t *val = base + port * TASvar_PORT_QCI_width + TASvar_QCI_config_space_lower + reg;
	if (TN_qci_verbose)
      printf ("reading address:0x%llx value:0x%llx\n", port * TASvar_PORT_QCI_width + TASvar_QCI_config_space_lower + reg,*val);
      if(TN_qci_MEMDUMP)printf("TNbar1 0x%lx w \n",port * TASvar_PORT_QCI_width + TASvar_QCI_config_space_lower + reg);
    return *val;
  }
}
//********************************************************************************************************************
...
*/
uint32_t
QCI_get_config2 (uint64_t reg, uint8_t port, uint8_t shadow)
{
  if (QCI_enable) {
...
*/
uint32_t
QCI_set_config(uint64_t reg, uint8_t port, uint32_t value)
{
  if (QCI_enable) {
    if (TN_qci_verbose)
      printf ("writing to address:0x%llx value:0x%llx\n", port * TASvar_PORT_QCI_width + TASvar_QCI_config_space_lower + reg,value);
      if(TN_qci_MEMDUMP)printf("TNbar1 0x%lx w 0x%lx\n",port * TASvar_PORT_QCI_width + TASvar_QCI_config_space_lower + reg,value);
    uint32_t *val = BASE + port * TASvar_PORT_QCI_width + TASvar_QCI_config_space_lower + reg;
    *val = value;
    uint32_t *val_shadow = BASE_shadow + port * TASvar_PORT_QCI_width + TASvar_QCI_config_space_lower + reg;
    *val = value;
  }
}
//********************************************************************************************************************
...
*/
uint32_t
QCI_set_config2(uint64_t reg, uint8_t port, uint32_t value)
{
  if (QCI_enable) {
    if (TN_qci_verbose)
      printf ("writing to address:0x%llx value:0x%llx\n", port * TASvar_PORT_QCI_width + TASvar_QCI_config_space_lower + reg,value);
...
*/
void
QCI_apply (struct arguments *arguments)
{
...
check_gcl(uint8_t port)
{
  ...
  gcl_length=QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN, port,0); // get gcl length
  for(i=0;i<gcl_length;i++){
  	struct GateControl_list_entry *entry_shadow =
      BASE_shadow + port * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * i;
    if (entry_shadow != NULL) {
...
get_real_cycletime(uint8_t port)
{
  ...
  for(i=0;i<gcl_length;i++){
 		 ...
 		 struct granularitytime granularitytime=get_QCI_granularitytime(port,i);	
...
check_tgcl(uint8_t port)
{
  ...
  for(i=0;i<gcl_length;i++){
  	struct GateControl_TIME_list_entry *entry =
	BASE_shadow + port * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * i;
      if (entry != NULL) {
...
void INR_QCI_correct_gcl(struct arguments *arguments){
	...
	uint32_t gcl_length=QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN, arguments->PORT,0); // get gcl length
	uint64_t i=0,j=0;
	...
	for(i=0;i<gcl_length;i++){ //get time error per cycle
		 gcl_correction_values=get_QCI_granularitytime(arguments->PORT, i);
		...
		for(i=0;i<gcl_length;i++){
		  ...
		  struct GateControl_list_entry *old_entry_shadow =
			...
			for(j=1;j<new_gcl_length;j++){
				...
				struct GateControl_TIME_list_entry *Tentry =
				  BASE + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * (i+j*gcl_length);
				struct GateControl_TIME_list_entry *Tentry_shadow =
				  BASE_shadow + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_TIME_list_base + TASvar_GateControl_TIME_list_entry_length * (i+j*gcl_length);
				struct GateControl_list_entry *entry =
				  BASE + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * (i+j*gcl_length);
				struct GateControl_list_entry *entry_shadow =
				  BASE_shadow + arguments->PORT * TASvar_PORT_QCI_width + TASvar_GateControl_list_base + TASvar_GateControl_list_entry_length * (i+j*gcl_length);
	...
	QCI_set_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN, arguments->PORT,new_gcl_length);//store new value
...
*/
void
new_QCI_apply (struct arguments *arguments){
    ...
    old_admin_basetime=arguments->ADMIN_BASE_TIME;
     if(arguments->PORT>=QCI_PORT_count){
		 ...
		 if(INR_TAS)QCI_set_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_GATE_ENABLE, arguments->PORT, 1);// enable TAS in case of INR version
...
/*    }*/
	...
	if (1 & QCI_get_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_GATE_ENABLE, arguments->PORT, 1)) {
      firstconfig = 0;
      verblog printf ("TSN already runing\n");
    }
    else {
      QCI_set_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_GATE_ENABLE, arguments->PORT, 1);
      firstconfig = 1;
      verblog printf ("TSN was off, enabling\n");
    }				//enable TSN
		
		
	verblog printf("GCL length:%i\n",QCI_get_config(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_GCL_LEN, arguments->PORT,0));	
	verblog printf ("Wait until pending config is done\n");
    while (QCI_get_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_CONFIG_CHANGE_PENDING, arguments->PORT, 0)) {
      POLLSLEEP}
...
start:	
	...
	if (arguments->hilscher_mode) {// wait until config change time is insede next running cycle
      TAS_OPER_BASE_TIME = 0xffffffff&tick2ns (QCI_get_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_OPER_BASE_TIME, arguments->PORT, 0));
...
startsleep:
	...
	TN_qci_config_change (arguments);//set values to hardware;
	
	QCI_set_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_CONFIG_CHANGE, arguments->PORT, 1);	//trigger config change
	//usleep(1);//be sure the TAS gets the falling edge
 QCI_set_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_CONFIG_CHANGE, arguments->PORT, 0);
/*    if(arguments->hilscher_mode){*/
/*    	printf ("writing oper config...\n");*/
/*		QCI_set_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_OPER_BASE_TIME, arguments->PORT, ns2ticks (arguments->ADMIN_BASE_TIME));*/
/*		QCI_set_config (TASvar_C_SUB_ADDR_TM_SCHED_TAS_OPER_CYCLE_TIME, arguments->PORT, ns2ticks (arguments->ADMIN_CYCLE_TIME));*/
/*    }*/
	check_gcl(arguments->PORT);// check if cosed queues are configured!
	QCI_set_config2(TASvar_C_SUB_ADDR_TM_SCHED_TAS_ADMIN_BASE_TIME,arguments->PORT,old_admin_basetime);
