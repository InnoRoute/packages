/**
*@file sh_mem.c
*@brief Enthält Funktionen zum Zugriff auf den Shared-memory
*/
#include <linux/ipc.h>
#include <linux/shm.h>
...
uint16_t *shmPtr;
/**
*@brief Kopiert einen Tiefenframe 320*240*2 bytes in den SharedMemory  
*@param *buf Rohdaten
...
{
  int p = 0;			// buf in sharedMem schreiben)
  uint8_t wdh = 1;
  while (wdh) {
    ...
    if (shmPtr == (uint16_t *) - 1) {
      ...
      shmPtr[(320 * 240) + 1] = flag;	//Flag übermitteln
...
cpfromshmem (uint16_t * buf, int shID, int semid)
{
  ...
  while (ret) {
    ...
    if (shmPtr == (uint16_t *) - 1) {
      ...
      flag = shmPtr[(320 * 240) + 1];	//flag lesen
      if (shmPtr[0] == 0) {
	int k;
	for (k = 0; k < 320 * 240; k++)
	  buf[k] = shmPtr[k + 1];
...
{
  int shid = 0;
  shid = shmget (SHMKEY, SHMSIZE, IPC_CREAT | 0666);	//create shmem
  return shid;
}
